...
public class RenderBrainEnergy extends Render {
	...
	public void doRender(Entity entity, double x, double y, double z, float yaw, float partialTick) {
		miniSun((EntityBrainEnergy)entity, 0.02F, x, y + 1, z, partialTick);
	}
    ...
    protected void miniSun(EntityBrainEnergy spark, float scale, double x, double y, double z, float partialTicks) {
    ...
    private void renderDragonDeath(EntityBrainEnergy spark, float density, float x, float y, float z, float partialTicks) {
		...
		for (int i = 0; (float) i < (f + f * f) / 2.0F * 60.0F; ++i) {
			...
			bufferbuilder.pos(0.0D, 0.0D, 0.0D).color(255, 255, 200, (int) (255.0F * (1.0F - f1))).endVertex();
