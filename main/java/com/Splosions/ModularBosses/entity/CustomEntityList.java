...
public class CustomEntityList
{
	...
	public static int getEntityId(Class<? extends Entity> oclass) {
		if (classToIDMapping.containsKey(oclass)) {
			...
			ModularBosses.logger.warn(String.format("ID Conflict: Entity ID %d already taken while getting ID for entity of class %s", entitiesAdded, oclass));
	...
	public static Entity createEntityFromNBT(NBTTagCompound compound, World world) {
		...
		if (entity != null) {
			try {
				...
				ModularBosses.logger.error(String.format("An Entity %s has thrown an exception during loading, its state cannot be restored. Report this to the mod author", compound.getString("id")));
				e.printStackTrace();
				entity = null;
			}
		} else {
			ModularBosses.logger.warn("Skipping Entity with id " + compound.getString("id"));
		}
